...
export class SymptomsComponent implements OnInit {
  ...
  constructor(public masterDataSvc:MasterDataService,private matSnackBar:MatSnackBar,) { 
    this.masterDataSvc.getAllMedicines().subscribe(result=>{
  ...
  switchToSymp(){
    ...
    this.masterDataSvc.getAllSymptoms().subscribe(result=>{
  ...
  onEditPress(selections,medForm,sympForm){
    this.editMode = true;
    let selected = selections.selectedOptions.selected[0].value;
    ...
    if(this.toolBtnMedicine){
      ...
      this.medicineFormGrp.controls.ingreds.setValue(selected.ingredients.toLocaleString());
    }else{
  ...
  deleteData(data:any){
    ...
    if(this.toolBtnMedicine){
      ...
      this.masterDataSvc.deleteMedicines(idArrs)
        .subscribe(result=>{
      ...
      this.masterDataSvc.deleteSymptoms(idArrs)
      .subscribe(result=>{
        this.matSnackBar.open("Success","OK");
        this.masterDataSvc.getAllSymptoms().subscribe(result=>{
          this.listData = result;
  ...
  onSymptomsSave(){
    ...
    if(this.medChips.length){
      ...
      if(this.editMode){
        this.masterDataSvc.updateSymptom(obj,this.elementId).subscribe(result=>{
          this.editMode = false;
          this.matSnackBar.open("Success","OK")
        },err=>{
          console.error(err);
        ...
        this.masterDataSvc.addNewSymtom(obj).subscribe(result=>{
          if(result.status){
  ...
  onMedicineSave(){
    ...
    let ingredients = this.medicineFormGrp.controls.ingreds.value;
    obj.ingredients = ingredients.split(",");
    if(this.editMode){
      this.masterDataSvc.updateMedicine(obj,this.elementId).subscribe(result=>{
        this.editMode = false;
        this.matSnackBar.open("Success","OK")
      },err=>{
        console.error(err);
      ...
      this.masterDataSvc.addNewMedicine(obj).subscribe(result=>{
        if(result.status){
          this.matSnackBar.open(result.status,"OK");
        }else{
          this.matSnackBar.open("Success","OK");
